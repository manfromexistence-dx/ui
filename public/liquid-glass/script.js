// The 'Pane' object is now available globally from the script tag as 'Tweakpane.Pane'
// The 'gsap' and 'Draggable' objects are available globally from their script tags.
...
// Manually created preset objects to avoid object spread '...'
var presets = {
  dock: {
    icons: true,
    scale: -180,
...
// Manually created config object
var config = {
  icons: true,
  scale: -180,
  radius: 16,
...
var ctrl = new Tweakpane.Pane({
  title: "config",
  expanded: true
});

// build this
var debugPen = document.querySelector(".displacement-debug");
var buildDisplacementImage = function () {
  var border = Math.min(config.width, config.height) * (config.border * 0.5);
  ...
  var kids =
    ...
    '" rx="' +
    config.radius +
    '" fill="url(#blue)" style="mix-blend-mode: ' +
    config.blend +
    '" />' +
    '   <rect x="' +
    border +
  ...
  var svgEl = debugPen.querySelector(".displacement-image");
...
ctrl
  ...
  .on("change", function () {
    ...
    if (config.preset !== "free") {
      var values = presets[config.preset];
      ...
      // Replaced for...of loop with a for...in loop
      for (var key in values) {
        if (Object.prototype.hasOwnProperty.call(values, key)) {
          var value = values[key];
          // Replaced computed property name with a helper object
          var animConfig = {};
          animConfig[key] = value;
          morph.to(config, animConfig, 0);
        }
      }
    }
  });
...
// Replaced destructuring assignment with separate variable assignments
var rect = document.querySelector(".dock-placeholder").getBoundingClientRect();
var top = rect.top;
var left = rect.left;
gsap.set(".effect", {
  top: top,
